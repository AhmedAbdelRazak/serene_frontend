...
const UpdateProductSingle = ({ productId }) => {
	...
	const gettingAllProducts = () => {
		getProducts().then((data) => {
			if (data.error) {
				...
				productAttributes = data.filter((e) => e._id === productId)[0]
					.productAttributes;

				setProductName(data.filter((e) => e._id === productId)[0].productName);
				setProductName_Arabic(
					data.filter((e) => e._id === productId)[0].productName_Arabic
				);
				setSizeChart(data.filter((e) => e._id === productId)[0].sizeChart);
				setSlug(data.filter((e) => e._id === productId)[0].slug);
				setSlug_Arabic(data.filter((e) => e._id === productId)[0].slug_Arabic);
				setMainColor(data.filter((e) => e._id === productId)[0].color);
				setMainSize(data.filter((e) => e._id === productId)[0].size);
				setProductSKU(data.filter((e) => e._id === productId)[0].productSKU);
				setDescription(data.filter((e) => e._id === productId)[0].description);
				setDescription_Arabic(
					data.filter((e) => e._id === productId)[0].description_Arabic
				);
				setChosenSubcategories(
					data.filter((e) => e._id === productId)[0].subcategory &&
						data
							.filter((e) => e._id === productId)[0]
							.subcategory.map((iii) => iii._id)
				);
				setChosenCategory(data.filter((e) => e._id === productId)[0].category);
				setChosenGender(data.filter((e) => e._id === productId)[0].gender);
				setAddThumbnail(
					data.filter((e) => e._id === productId)[0].thumbnailImage[0].images[0]
						? data.filter((e) => e._id === productId)[0].thumbnailImage[0]
						: []
				);

				setPrice(data.filter((e) => e._id === productId)[0].price);
				setPriceAfterDiscount(
					data.filter((e) => e._id === productId)[0].priceAfterDiscount
				);
				setMSRPPriceBasic(
					data.filter((e) => e._id === productId)[0].MSRPPriceBasic
				);
				setStock(data.filter((e) => e._id === productId)[0].quantity);

				setChosenSeason(
					data.filter((e) => e._id === productId)[0].chosenSeason
				);

				setViewsCount(data.filter((e) => e._id === productId)[0].viewsCount);

				setAddVariables(
					data.filter((e) => e._id === productId)[0].addVariables
				);
				setClearance(data.filter((e) => e._id === productId)[0].clearance);
				setActiveBackorder(
					data.filter((e) => e._id === productId)[0].activeBackorder
						? data.filter((e) => e._id === productId)[0].activeBackorder
						: false
				);
				setShipping(data.filter((e) => e._id === productId)[0].shipping);
				setActiveProduct(
					data.filter((e) => e._id === productId)[0].activeProduct
				);
				setFeatured(data.filter((e) => e._id === productId)[0].featuredProduct);
				setStoreName(data.filter((e) => e._id === productId)[0].storeName);
				setProductAttributesFinal(
					data.filter((e) => e._id === productId)[0].productAttributes
				);
				...
				getListOfSubs(
					data.filter((e) => e._id === productId)[0].category._id
				).then((data) => {
					if (data.error) {
						console.log(data.error);
					} else {
				...
				setPolicy(data.filter((e) => e._id === productId)[0].policy);
				setPolicy_Arabic(
				...
				setSpecs_Arabic(
					data.filter((e) => e._id === productId)[0].Specs_Arabic
				);
				setFitCare(data.filter((e) => e._id === productId)[0].fitCare);
				setFitCare_Arabic(
					data.filter((e) => e._id === productId)[0].fitCare_Arabic
				);

				setRelatedProducts(
					data.filter((e) => e._id === productId)[0].relatedProducts &&
						data
							.filter((e) => e._id === productId)[0]
	...
	const UpdateProductToDatabase = (e) => {
		...
		updateProduct(productId, user._id, token, {
			product: values,
		}).then((data) => {
			if (data.error) {
				console.log(data.error);
	...
	const extraFeatures = () => {
		return (
			<form className='mt-4 ml-5'>
				...
				{/* Multi-select dropdown for related products */}
				<div className='form-group mt-5'>
					<label
						className='text-muted'
						style={{ fontWeight: "bold", fontSize: "17px" }}
